...
public interface CommonDao {
    ...
    @Select("select * from ${tableName} where 1=1 ${where} limit #{start},#{size}") // where ==> and name = xx and password = xx
    @ResultType(HashMap.class)
    ...
    @Select("select count(*) from ${tableName} where 1=1 ${where}") //where ==> and name = xx and password = xx
    @ResultType(Integer.class)
    int listCountForWhere(@Param("tableName") String tableName,@Param("where")  String where);

    @Update("update ${tableName} set ${sets} where 1=1 ${where}") //sets ==> name=xx,password=xx
    @ResultType(Integer.class)
